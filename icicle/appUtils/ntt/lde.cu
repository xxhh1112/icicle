...
int evaluate_batch(E *d_out, E *d_coefficients, S *d_domain, unsigned domain_size, unsigned n, unsigned batch_size, bool coset, S *coset_powers)
{
  ...
  S tw12;
  int tw_off = n / 8;
  cudaMemcpy(&tw4, &d_domain[tw_off], sizeof(S), cudaMemcpyDeviceToHost);
  cudaMemcpy(&tw8, &d_domain[2*tw_off], sizeof(S), cudaMemcpyDeviceToHost);
  cudaMemcpy(&tw12, &d_domain[3*tw_off], sizeof(S), cudaMemcpyDeviceToHost);

  ntt_template_kernel_shared_rev<<<NUM_BLOCKS, NUM_THREADS, shared_mem, 0>>>(d_out, 1 << logn_shmem, d_domain, n / 2, total_tasks, 0, logn_shmem - 1, n_div_log2_blocks, num_blocks2x, (1 << logn_shmem) - 1, tw4, tw8, tw12);
...
S tw4;
  ...
  S tw12;
  int tw_off = n / 8;
  cudaMemcpy(&tw4, &d_twf[tw_off], sizeof(S), cudaMemcpyDeviceToHost);
  cudaMemcpy(&tw8, &d_twf[2*tw_off], sizeof(S), cudaMemcpyDeviceToHost);
  cudaMemcpy(&tw12, &d_twf[3*tw_off], sizeof(S), cudaMemcpyDeviceToHost);

  ntt_template_kernel_shared_rev<<<NUM_BLOCKS, NUM_THREADS, shared_mem, 0>>>(d_inout, 1 << logn_shmem, d_twf, n / 2, total_tasks, 0, logn_shmem - 1, n_div_log2_blocks, num_blocks2x, (1 << logn_shmem) - 1, tw4, tw8, tw12);
