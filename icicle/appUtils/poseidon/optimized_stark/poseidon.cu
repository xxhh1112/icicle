...
template <typename S>
__device__ __forceinline__ S sbox_alpha_cube(S element)
{
  ...
  const PoseidonConfiguration<S> config)
{
  ...
  element = sbox_alpha_cube(element);
  if (!skip_rc) {
    element = element + config.round_constants[rc_offset + element_number];
  }
...
__device__ S partial_round(S* state, size_t rc_offset, int round_number, const PoseidonConfiguration<S> config)
{
  ...
  S* sparse_matrix = &config.sparse_matrices[(config.t * config.t) * round_number];
  ...
  for (int i = 1; i < config.t; i++) {
    state[i] = state[i] + (element * sparse_matrix[config.t * i]);
  }
}
