...
template <typename S>
class StarkPoseidon: public Poseidon<S>
{
public:
  PoseidonConfiguration<S> config;
  ParallelPoseidonConfiguration kernel_params;

  StarkPoseidon(const uint32_t arity, cudaStream_t stream) : Poseidon<S>(arity), kernel_params(arity + 1) {
    config.t = arity + 1;
    this->stream = stream;
    ...
    uint32_t sparse_matrices_len = config.t * config.t * config.partial_rounds;

    S* round_constants = load_round_constants<S>(arity);
    S* mds_matrix = load_mds_matrix<S>(arity);
    S* non_sparse_matrix = load_pre_mds_matrix<S>(arity);
    S* sparse_matrices = load_sparse_matrices<S>(arity);
    ...
    cudaMemcpyAsync(
      config.mds_matrix, mds_matrix, sizeof(S) * mds_matrix_len, cudaMemcpyHostToDevice, stream_copy_mds_matrix);
    cudaMemcpyAsync(
      config.non_sparse_matrix, non_sparse_matrix, sizeof(S) * mds_matrix_len, cudaMemcpyHostToDevice,
      stream_copy_non_sparse);
    cudaMemcpyAsync(
      config.sparse_matrices, sparse_matrices, sizeof(S) * sparse_matrices_len, cudaMemcpyHostToDevice,
      stream_copy_sparse_matrices);
  ...
  void permute_many(S * states, size_t number_of_states, cudaStream_t stream) override {
    ...
    std::cout << "Initial RCOFFSET: " << rc_offset << std::endl;
